...
async function main() {
  ...
  const bytecode = fs.readFileSync("./artifacts/contracts/ThirdPartyPayment.sol/ThirdPartyPayment.bin");
  const contractCreate = new ContractCreateFlow().setGas(3000000).setBytecode(bytecode);
  const txResponse = await contractCreate.execute(client);
  const receipt = await txResponse.getReceipt(client);
  const contractId = receipt.contractId;
  console.log("ThirdPartyPayment Contract deployed to:", contractId.toString());
  await client.close();
}
